...
- [ ] Delete IN/OUT support
  - [ ] Remove `IIOBus.cs`
  - [ ] Remove the `_io` field and the `IIOBus` constructor parameter on `Cpu`
  - [ ] Remove `In()` (`0xDB`) and `Out()` (`0xD3`), and their dispatch arms
  - [ ] Create `Cpu.Interrupts.cs` and move `Di()` / `Ei()` into it
    (dispatch arms `0xF3` / `0xFB` keep pointing at the same method names).
    Then delete `Cpu.Io.cs`.
- [ ] Remove `Xchg` and `Xthl` from `Cpu.Stack.cs` (keep the file — `PushB`,
  `PopB`, …, `PushPsw`, `PopPsw`, `Sphl` all stay)
- [ ] Remove the legacy interrupt-injection plumbing from `Cpu.cs`
  - [ ] Field `_isInterruptPending`
  - [ ] Field `_pendingInterruptOpcode`
  - [ ] Public `Interrupt(byte)` API
  - [ ] Method `TryExecuteInterrupt`
  - [ ] The corresponding two lines in `Reset()`
  - [ ] The `TryExecuteInterrupt` call in `Step()` (leave the `Halted` /
...
- [ ] Remove "undocumented alias" entries from the `Execute` switch
  - [ ] `0x08, 0x10, 0x18, 0x20, 0x28, 0x30, 0x38` (currently → `Nop`)
  - [ ] `0xCB` (currently → `Jmp`; on LR35902 it is the CB-prefix, wired in
    step 4)
  - [ ] `0xD9` (currently → `Ret`; on LR35902 it is `RETI`, wired in step 5)
  - [ ] `0xDD, 0xED, 0xFD` (currently → `Call`; permanently illegal on
    LR35902)
- [ ] Remove the existing 8080 dispatch arms whose opcode bytes are reused,
      and delete the now-orphaned handlers
  - [ ] `0x22 Shld`, `0x2A Lhld`, `0x32 StA`, `0x3A LdA` — also delete
  ...
  - [ ] `0xE3 Xthl`, `0xEB Xchg` — dispatch arms removed alongside the
    handlers above.

### Tests

- [ ] Delete `GameboyEmulator.Core.Tests/CpuIoTests.cs` (deferred from step 1).
- [ ] Remove `NoOpCpuIO` from `CpuTestHelper.cs`.
- [ ] Update `CpuTestBase.cs`
  - [ ] Drop the `new NoOpCpuIO()` argument from the `Cpu` constructor call
  - [ ] Drop or update `CreateCpu(CpuState, IIOBus)` (no longer needed)
- [ ] Delete `TestXthl` and `TestXchg` in `CpuMovTests.cs` (these are the
  only tests that exercise the removed handlers).
...
- The 11 LR35902 permanently-illegal opcodes (`0xD3, 0xDB, 0xDD, 0xE3, 0xE4,
  ...
  fault explicitly.
- Test suite still compiles and is green (the only deleted tests are the four
  listed above, plus the entire `CpuIoTests` file).
